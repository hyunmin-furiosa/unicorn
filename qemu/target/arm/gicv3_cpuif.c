...
// Ref. https://developer.arm.com/documentation/ihi0069/latest/
// 12.2 AArch64 System register descriptions
uint64_t encode_cpreg(const ARMCPRegInfo *ri);
uint64_t encode_cpreg(const ARMCPRegInfo *ri)
{
    return (uint64_t)(ri->opc0 & 0x3) << 14 | (uint64_t)(ri->opc1 & 0x7) << 11 |
           (uint64_t)(ri->crn & 0xf) << 7 | (uint64_t)(ri->crm & 0xf) << 3 |
           (uint64_t)(ri->opc2 & 0x7) << 0;
}

static uint64_t gicv3_reg_read(CPUARMState *env, const ARMCPRegInfo *ri)
{
    uc_engine *uc = env->uc;
    ...
    // fprintf(stderr, "gicv3_reg_read %s @ 0x%016lx\n", ri->name, tx.addr);
    func(env->uc, opaque, &tx);

    return val;
}

static void gicv3_reg_write(CPUARMState *env, const ARMCPRegInfo *ri,
                            uint64_t val)
{
    uc_engine *uc = env->uc;
    uc_cb_mmio_t func = uc->uc_portio_func;
    ...
    // fprintf(stderr, "gicv3_reg_write %s @ 0x%016lx pc = 0x%016lx\n",
    // ri->name, tx.addr, env->pc);
    func(env->uc, opaque, &tx);
}

static CPAccessResult gicv3_reg_access(CPUARMState *env, const ARMCPRegInfo *ri,
                                       bool isread)
{
...
static CPAccessResult gicv3_fiq_access(CPUARMState *env, const ARMCPRegInfo *ri,
                                       bool isread)
{
    CPAccessResult r = CP_ACCESS_OK;
...
static CPAccessResult gicv3_irq_access(CPUARMState *env, const ARMCPRegInfo *ri,
                                       bool isread)
{
    CPAccessResult r = CP_ACCESS_OK;
    // GICv3CPUState *cs = icc_cs_from_env(env);
...
static const ARMCPRegInfo gicv3_cpuif_reginfo[] = {
    {
        ...
        .access = PL1_RW,
        .accessfn = gicv3_reg_access,
        .readfn = gicv3_reg_read,
        .writefn = gicv3_reg_write,
    },
    {
        .name = "ICC_IAR0_EL1",
        .state = ARM_CP_STATE_BOTH,
